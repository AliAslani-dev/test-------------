...
import { adminBucketToggleEnabled } from '@/api/admin/bucket/service';
...
export default function BucketTableEnabledCell({ value, row, onLocalToggle }: Props) {
  const { lang } = useLang();
      ...
      <span>
        <Chip
          ...
          onClick={async () => {
            ...
            setBusy(true);
            try {
              await adminBucketToggleEnabled(row.id);
