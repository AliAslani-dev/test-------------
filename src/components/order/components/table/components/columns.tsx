...
import { wholesaleOrderTableColumnsName } from '@/components/order/data';
...
export const makeWholesaleOrderTableColumns = (
  ...
  onEditRow?: (row: WholesaleOrderTable) => void,
  editingOrderId?: number | null,
): Column<WholesaleOrderTable>[] => [
  {
    id: 'rowNumber',
    label: wholesaleOrderTableColumnsName['rowNumber'],
    align: 'right',
    minWidth: 24,
  ...
  {
    id: 'zarplusUser',
    label: wholesaleOrderTableColumnsName['zarplusUser'],
    minWidth: 150,
    align: 'right',
  },
  {
  ...
  },
  {
    ...
    sortable: false,
    render: (_v, row) => (
      <WholesaleOrderTableActionsCell
        status={row.status}
        loading={editingOrderId === row.id}
