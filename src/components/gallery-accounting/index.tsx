...
interface GalleryAccountingTabProps {
  userId: number | null;
  zarplusUserId: number;
  role: UserRole;
}

const GalleryAccountingTab: FunctionComponent<GalleryAccountingTabProps> = ({
  userId,
  zarplusUserId,
  role,
}) => {
  ...
  const { lang } = useLang();
  const { t } = useText('galleryAccounting', lang);
  ...
  const [accountingData, setAccountingData] = useState<GalleryAccountingDTO[]>([]);

  const [providerDialogOpen, setProviderDialogOpen] = useState(false);
  const [provider, setProvider] = useState<UserTable | undefined>(undefined);

  const [zarplusUserDialogOpen, setZarplusUserDialogOpen] = useState(false);
  const [zarplusUser, setZarplusUser] = useState<ZarplusUserDTO | undefined>(undefined);
  ...
  const handleCloseZarplusDialog = () => {
    setZarplusUserDialogOpen(false);
  };
      ...
      if ((isAdmin && userId) || (!isAdmin && user)) {
        const [result, userRes, accRes] = await Promise.all([
          getOrders(isAdmin ? (userId as number) : (user?.id as number), {
            zarplus_user_id: zarplusUserId,
          }),
          getZarplusUsers(),
          getGalleryAccounting(isAdmin ? (userId as number) : (user?.id as number), zarplusUserId),
        ]);
  ...
  const accountingCards = useMemo(() => {
    ...
    return [
      {
        key: 'asset_credit',
        title: t('asset_credit'),
        value: dataFetched ? String(tPD(base.assetCredit / precisionFactor)) : '۰',
        type: 'gold',
      },
      {
        key: 'rial_credit',
        title: t('rial_credit'),
        value: dataFetched ? String(tPD(fWC(base.rialCredit))) : '۰',
        type: 'rial',
      },
    ];
  }, [accountingData, t]);
  ...
  return (
    <Box sx={SX.tab_wrapper}>
      ...
      <ZarplusUserDialog
        open={zarplusUserDialogOpen}
        onClose={handleCloseZarplusDialog}
        showingUser={zarplusUser}
      />

      {/* --- New Stacked Header UI --- */}
      <Box sx={SX.header_container}>
        <Typography sx={SX.tab_title}>{t('tab_title')}</Typography>
        ...
        <Box sx={SX.context_badges_container}>
          ...
          {zarplusUser?.username && (
            <Box sx={SX.user_badge}>
              <Typography sx={SX.user_badge_label}>گالری:</Typography>
              <Typography sx={SX.user_badge_value} onClick={() => setZarplusUserDialogOpen(true)}>
                {zarplusUser.username}
              </Typography>
            </Box>
          )}
...
export default GalleryAccountingTab;
